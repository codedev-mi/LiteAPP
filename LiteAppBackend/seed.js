...
async function main() {
  ...
  // Clear existing items (be careful in production)
  ...
  const categoryNames = ['Fruits', 'Vegetables', 'Grocery', 'Dairy', 'Meat / Seafood'];
  ...
  const upi = await prisma.paymentMethod.create({
    data: { type: 'UPI', name: 'Google Pay / PhonePe', detail: 'user@okaxis', icon: '💳' }
  });
  const card = await prisma.paymentMethod.create({
    data: { type: 'CARD', name: 'Credit / Debit Card', detail: '**** **** **** 4532', icon: '🏦' }
  });
  const cod = await prisma.paymentMethod.create({
    data: { type: 'COD', name: 'Cash on Delivery', detail: 'Pay at your doorstep', icon: '🏠' }
  });

  // Create Promo Code
  await prisma.promoCode.deleteMany();
  const promo = await prisma.promoCode.create({
    data: { code: 'FRESH50', discountAmount: 50, minOrderValue: 200, isActive: true }
  });

  // Create Banners
  await prisma.banner.deleteMany();
  const b1 = await prisma.banner.create({
    data: { title: 'Fresh Summer Fruits', subtitle: 'Flat 20% Off', imageUrl: 'https://images.unsplash.com/photo-1610832958506-aa56368176cf?q=80&w=1000&auto=format&fit=crop' }
  });
  const b2 = await prisma.banner.create({
    data: { title: 'Organic Essentials', subtitle: 'Buy 1 Get 1 Free', imageUrl: 'https://images.unsplash.com/photo-1542838132-92c53300491e?q=80&w=1000&auto=format&fit=crop' }
  });
